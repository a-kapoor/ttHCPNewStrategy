...
class plotter(object):
    ...
    def plot_training_progress_acc(self, histories, labels):
        ...
        acc_title = 'plots/DNN_accuracy_wrt_epoch.pdf'
        plt.tight_layout()
        return
    ...
    def draw_category_overfitting_plot(self, y_scores_train, y_scores_test, plot_info):
        ...
        name = list(filter(str.isalnum, str(data_type).split(".")[-1]))
        ...
        for index in range(len(y_scores_train)):
            ...
            if index == 3:
                histo_train_newphysics = histo_train_ / np.sum(histo_train_, dtype=np.float32)
            ...
            if np.sum(histo_test_, dtype=np.float32) <= 0 :
                ...
                if index == 3:
                    histo_test_newphysics = histo_test_
                ...
                if index == 3:
                    histo_test_newphysics = histo_test_ / np.sum(histo_test_, dtype=np.float32)
        ...
        if 'Other' in node_name:
            ...
            train_Othervnewphysics = "{0:.5g}".format(self.GetSeparation(histo_train_Other,histo_train_newphysics))
            ...
            test_Othervnewphysics = "{0:.5g}".format(self.GetSeparation(histo_test_Other,histo_test_newphysics))
            ...
            Other_v_newphysics_train_sep = 'Other vs newphysics train Sep.: %s' % ( train_Othervnewphysics )
            self.ax.annotate(Other_v_newphysics_train_sep,  xy=(0.7, 1.75), xytext=(0.7, 1.75), fontsize=9)
            ...
            Other_v_newphysics_test_sep = 'Other vs newphysics test Sep.: %s' % ( test_Othervnewphysics )
            self.ax.annotate(Other_v_newphysics_test_sep,  xy=(0.7, 1.), xytext=(0.7, 1.), fontsize=9)


            separations_forTable = r'''%s & \textbackslash & %s & %s''' % (test_OthervttW,Other_v_tHQ_test_sep, Other_v_newphysics_test_sep)
        ...
        if 'ttW' in node_name:
            ...
            train_ttWvnewphysics = "{0:.5g}".format(self.GetSeparation(histo_train_ttW,histo_train_newphysics))
            ...
            test_ttWvnewphysics = "{0:.5g}".format(self.GetSeparation(histo_test_ttW,histo_test_newphysics))
            ...
            ttW_v_tHQ_train_sep = 'ttW vs tHQ train Sep.: %s' % ( train_ttWvtHQ )
            self.ax.annotate(ttW_v_tHQ_train_sep,  xy=(0.7, 1.75), xytext=(0.7, 1.75), fontsize=9)
            ttW_v_newphysics_train_sep = 'ttW vs newphysics train Sep.: %s' % ( train_ttWvnewphysics )
            self.ax.annotate(ttW_v_newphysics_train_sep,  xy=(0.7, 1.75), xytext=(0.7, 1.75), fontsize=9)
            ...
            ttW_v_newphysics_test_sep = 'ttW vs newphysics test Sep.: %s' % ( test_ttWvnewphysics )
            self.ax.annotate(ttW_v_newphysics_test_sep,  xy=(0.7, 1.), xytext=(0.7, 1.), fontsize=9)

            separations_forTable = r'''%s & %s & \textbackslash & %s''' % (test_ttWvOther,ttW_v_tHQ_test_sep, ttW_v_newphysics_test_sep)
        ...
        if 'tHQ' in node_name:
            ...
            train_tHQvnewphysics = "{0:.5g}".format(self.GetSeparation(histo_train_tHQ,histo_train_newphysics))
            ...
            test_tHQvnewphysics = "{0:.5g}".format(self.GetSeparation(histo_test_tHQ,histo_test_newphysics))
            ...
            tHQ_v_newphysics_train_sep = 'tHQ vs newphysics train Sep.: %s' % ( train_tHQvnewphysics )
            self.ax.annotate(tHQ_v_newphysics_train_sep,  xy=(0.7, 2.), xytext=(0.7, 2.), fontsize=9)
            ...
            tHQ_v_newphysics_test_sep = 'tHQ vs newphysics test Sep.: %s' % ( test_tHQvnewphysics )
            self.ax.annotate(tHQ_v_newphysics_test_sep,  xy=(0.7, 1.25), xytext=(0.7, 1.25), fontsize=9)

            separations_forTable = r'''%s & %s & %s \textbackslash ''' % (test_tHQvOther,tHQ_v_ttW_test_sep, tHQ_v_newphysics_test_sep)
        



        if 'newphysics' in node_name:
            train_newphysicsvOther = "{0:.5g}".format(self.GetSeparation(histo_train_newphysics,histo_train_Other))
            train_newphysicsvttW = "{0:.5g}".format(self.GetSeparation(histo_train_newphysics,histo_train_ttW))
            train_newphysicsvtHQ = "{0:.5g}".format(self.GetSeparation(histo_train_newphysics,histo_train_tHQ))

            test_newphysicsvOther = "{0:.5g}".format(self.GetSeparation(histo_test_newphysics,histo_test_Other))
            test_newphysicsvttW = "{0:.5g}".format(self.GetSeparation(histo_test_newphysics,histo_test_ttW))
            test_newphysicsvtHQ = "{0:.5g}".format(self.GetSeparation(histo_test_newphysics,histo_test_tHQ))

            newphysics_v_Other_train_sep = 'newphysics vs Other train Sep.: %s' % ( train_newphysicsvOther )
            self.ax.annotate(newphysics_v_Other_train_sep,  xy=(0.7, 2.25), xytext=(0.7, 2.25), fontsize=9)
            newphysics_v_ttW_train_sep = 'newphysics vs ttW train Sep.: %s' % ( train_newphysicsvttW )
            self.ax.annotate(newphysics_v_ttW_train_sep,  xy=(0.7, 2.), xytext=(0.7, 2.), fontsize=9)
            newphysics_v_tHQ_train_sep = 'newphysics vs tHQ train Sep.: %s' % ( train_newphysicsvtHQ )
            self.ax.annotate(newphysics_v_tHQ_train_sep,  xy=(0.7, 2.), xytext=(0.7, 2.), fontsize=9)

            newphysics_v_Other_test_sep = 'newphysics vs Other test Sep.: %s' % ( test_newphysicsvOther )
            self.ax.annotate(newphysics_v_Other_test_sep,  xy=(0.7, 1.5), xytext=(0.7, 1.5), fontsize=9)
            newphysics_v_ttW_test_sep = 'newphysics vs ttW test Sep.: %s' % ( test_newphysicsvttW )
            self.ax.annotate(newphysics_v_ttW_test_sep,  xy=(0.7, 1.25), xytext=(0.7, 1.25), fontsize=9)
            newphysics_v_tHQ_test_sep = 'newphysics vs tHQ test Sep.: %s' % ( test_newphysicsvtHQ )
            self.ax.annotate(newphysics_v_tHQ_test_sep,  xy=(0.7, 1.25), xytext=(0.7, 1.25), fontsize=9)


            separations_forTable = r'''%s & %s & %s \textbackslash ''' % (test_newphysicsvOther,newphysics_v_ttW_test_sep, newphysics_v_tHQ_test_sep)
        ...
        overfitting_plot_file_name = 'overfitting_plot_%s_%s.pdf' % (node_name,plot_title)
...
'''
    ...
    def overfitting(self, estimator, Y_train, Y_test, result_probs, result_probs_test, plots_dir, train_weights, test_weights, nbins=50):
        ...
        # Arrays to store tHQ categorised event
        ...
        for i in range(len(result_probs)):
            ...
            if Y_train[i][0] == 1:
                y_scores_train_Other_sample_Othernode.append(result_probs[i][0])
                y_scores_train_Other_sample_ttWnode.append(result_probs[i][1])
                y_scores_train_Other_sample_tHQnode.append(result_probs[i][2])
                y_scores_train_Other_sample_newphysicsnode.append(result_probs[i][3])
                if np.argmax(result_probs[i]) == 0:
                    y_scores_train_Other_sample_Other_categorised.append(result_probs[i][0])
                if np.argmax(result_probs[i]) == 1:
                    y_scores_train_Other_sample_ttW_categorised.append(result_probs[i][1])
                if np.argmax(result_probs[i]) == 2:
                    y_scores_train_Other_sample_tHQ_categorised.append(result_probs[i][2])
                if np.argmax(result_probs[i]) == 3:
                    y_scores_train_Other_sample_newphysics_categorised.append(result_probs[i][3])

            if Y_train[i][1] == 1:
                y_scores_train_ttW_sample_Othernode.append(result_probs[i][0])
                y_scores_train_ttW_sample_ttWnode.append(result_probs[i][1])
                y_scores_train_ttW_sample_tHQnode.append(result_probs[i][2])
                y_scores_train_ttW_sample_newphysicsnode.append(result_probs[i][3])
                if np.argmax(result_probs[i]) == 0:
                    y_scores_train_ttW_sample_Other_categorised.append(result_probs[i][0])
                if np.argmax(result_probs[i]) == 1:
                    y_scores_train_ttW_sample_ttW_categorised.append(result_probs[i][1])
                if np.argmax(result_probs[i]) == 2:
                    y_scores_train_ttW_sample_tHQ_categorised.append(result_probs[i][2])
                if np.argmax(result_probs[i]) == 3:
                    y_scores_train_ttW_sample_newphysics_categorised.append(result_probs[i][3])
            if Y_train[i][2] == 1:
                y_scores_train_tHQ_sample_Othernode.append(result_probs[i][0])
                y_scores_train_tHQ_sample_ttWnode.append(result_probs[i][1])
                y_scores_train_tHQ_sample_tHQnode.append(result_probs[i][2])
                y_scores_train_tHQ_sample_newphysicsnode.append(result_probs[i][3])
                if np.argmax(result_probs[i]) == 0:
                    y_scores_train_tHQ_sample_Other_categorised.append(result_probs[i][0])
                if np.argmax(result_probs[i]) == 1:
                    y_scores_train_tHQ_sample_ttW_categorised.append(result_probs[i][1])
                if np.argmax(result_probs[i]) == 2:
                    y_scores_train_tHQ_sample_tHQ_categorised.append(result_probs[i][2])
                if np.argmax(result_probs[i]) == 3:
                    y_scores_train_tHQ_sample_newphysics_categorised.append(result_probs[i][3])
            
            if Y_train[i][3] == 1:
                y_scores_train_newphysics_sample_Othernode.append(result_probs[i][0])
                y_scores_train_newphysics_sample_ttWnode.append(result_probs[i][1])
                y_scores_train_newphysics_sample_tHQnode.append(result_probs[i][2])
                y_scores_train_newphysics_sample_newphysicsnode.append(result_probs[i][3])
                if np.argmax(result_probs[i]) == 0:
                    y_scores_train_newphysics_sample_Other_categorised.append(result_probs[i][0])
                if np.argmax(result_probs[i]) == 1:
                    y_scores_train_newphysics_sample_ttW_categorised.append(result_probs[i][1])
                if np.argmax(result_probs[i]) == 2:
                    y_scores_train_newphysics_sample_tHQ_categorised.append(result_probs[i][2])
                if np.argmax(result_probs[i]) == 3:
                    y_scores_train_newphysics_sample_newphysics_categorised.append(result_probs[i][3])
        ...
        for i in range(len(result_probs_test)):
            ...
            if Y_test[i][0] == 1:
                y_scores_test_Other_sample_Othernode.append(result_probs_test[i][0])
                y_scores_test_Other_sample_ttWnode.append(result_probs_test[i][1])
                y_scores_test_Other_sample_tHQnode.append(result_probs_test[i][2])
                y_scores_test_Other_sample_newphysicsnode.append(result_probs_test[i][3])
                if np.argmax(result_probs_test[i]) == 0:
                    y_scores_test_Other_sample_Other_categorised.append(result_probs_test[i][0])
                if np.argmax(result_probs_test[i]) == 1:
                    y_scores_test_Other_sample_ttW_categorised.append(result_probs_test[i][1])
                if np.argmax(result_probs_test[i]) == 2:
                    y_scores_test_Other_sample_tHQ_categorised.append(result_probs_test[i][2])
                if np.argmax(result_probs_test[i]) == 3:
                    y_scores_test_Other_sample_newphysics_categorised.append(result_probs_test[i][3])
            if Y_test[i][1] == 1:
                y_scores_test_ttW_sample_Othernode.append(result_probs_test[i][0])
                y_scores_test_ttW_sample_ttWnode.append(result_probs_test[i][1])
                y_scores_test_ttW_sample_tHQnode.append(result_probs_test[i][2])
                y_scores_test_ttW_sample_newphysicsnode.append(result_probs_test[i][3])
                if np.argmax(result_probs_test[i]) == 0:
                    y_scores_test_ttW_sample_Other_categorised.append(result_probs_test[i][0])
                if np.argmax(result_probs_test[i]) == 1:
                    y_scores_test_ttW_sample_ttW_categorised.append(result_probs_test[i][1])
                if np.argmax(result_probs_test[i]) == 2:
                    y_scores_test_ttW_sample_tHQ_categorised.append(result_probs_test[i][2])
                if np.argmax(result_probs_test[i]) == 3:
                    y_scores_test_ttW_sample_newphysics_categorised.append(result_probs_test[i][3])

            if Y_test[i][2] == 1:
                y_scores_test_tHQ_sample_Othernode.append(result_probs_test[i][0])
                y_scores_test_tHQ_sample_ttWnode.append(result_probs_test[i][1])
                y_scores_test_tHQ_sample_tHQnode.append(result_probs_test[i][2])
                y_scores_test_tHQ_sample_newphysicsnode.append(result_probs_test[i][3])
                if np.argmax(result_probs_test[i]) == 0:
                    y_scores_test_tHQ_sample_Other_categorised.append(result_probs_test[i][0])
                if np.argmax(result_probs_test[i]) == 1:
                    y_scores_test_tHQ_sample_ttW_categorised.append(result_probs_test[i][1])
                if np.argmax(result_probs_test[i]) == 2:
                    y_scores_test_tHQ_sample_tHQ_categorised.append(result_probs_test[i][2])
                if np.argmax(result_probs_test[i]) == 3:
                    y_scores_test_tHQ_sample_newphysics_categorised.append(result_probs_test[i][3])

 
            if Y_test[i][3] == 1:
                y_scores_test_newphysics_sample_Othernode.append(result_probs_test[i][0])
                y_scores_test_newphysics_sample_ttWnode.append(result_probs_test[i][1])
                y_scores_test_newphysics_sample_tHQnode.append(result_probs_test[i][2])
                y_scores_test_newphysics_sample_newphysicsnode.append(result_probs_test[i][3])
                if np.argmax(result_probs_test[i]) == 0:
                    y_scores_test_newphysics_sample_Other_categorised.append(result_probs_test[i][0])
                if np.argmax(result_probs_test[i]) == 1:
                    y_scores_test_newphysics_sample_ttW_categorised.append(result_probs_test[i][1])
                if np.argmax(result_probs_test[i]) == 2:
                    y_scores_test_newphysics_sample_tHQ_categorised.append(result_probs_test[i][2])
                if np.argmax(result_probs_test[i]) == 3:
                    y_scores_test_newphysics_sample_newphysics_categorised.append(result_probs_test[i][3])
        ...
        self.yscores_train_categorised[0] = [y_scores_train_Other_sample_Other_categorised, y_scores_train_ttW_sample_Other_categorised, y_scores_train_tHQ_sample_Other_categorised, y_scores_train_newphysics_sample_Other_categorised]
        self.yscores_train_categorised[1] = [y_scores_train_Other_sample_ttW_categorised, y_scores_train_ttW_sample_ttW_categorised, y_scores_train_tHQ_sample_ttW_categorised, y_scores_train_newphysics_sample_ttW_categorised]
        self.yscores_train_categorised[2] = [y_scores_train_Other_sample_tHQ_categorised, y_scores_train_ttW_sample_tHQ_categorised, y_scores_train_tHQ_sample_tHQ_categorised, y_scores_train_newphysics_sample_tHQ_categorised]
        self.yscores_train_categorised[3] = [y_scores_train_Other_sample_newphysics_categorised, y_scores_train_ttW_sample_newphysics_categorised, y_scores_train_tHQ_sample_newphysics_categorised, y_scores_train_newphysics_sample_newphysics_categorised]

        self.yscores_test_categorised[0] = [y_scores_test_Other_sample_Other_categorised, y_scores_test_ttW_sample_Other_categorised, y_scores_test_tHQ_sample_Other_categorised, y_scores_test_newphysics_sample_Other_categorised]
        self.yscores_test_categorised[1] = [y_scores_test_Other_sample_ttW_categorised, y_scores_test_ttW_sample_ttW_categorised, y_scores_test_tHQ_sample_ttW_categorised, y_scores_test_newphysics_sample_ttW_categorised]
        self.yscores_test_categorised[2] = [y_scores_test_Other_sample_tHQ_categorised, y_scores_test_ttW_sample_tHQ_categorised, y_scores_test_tHQ_sample_tHQ_categorised, y_scores_test_newphysics_sample_tHQ_categorised]
        self.yscores_test_categorised[3] = [y_scores_test_Other_sample_newphysics_categorised, y_scores_test_ttW_sample_newphysics_categorised, y_scores_test_tHQ_sample_newphysics_categorised, y_scores_test_newphysics_sample_newphysics_categorised]

        #y_scores_train_nonCat = [[0 for x in range(w)] for y in range(h)]
        #y_scores_test_nonCat = [[0 for x in range(w)] for y in range(h)]
        self.yscores_train_non_categorised[0] = [y_scores_train_Other_sample_Othernode, y_scores_train_ttW_sample_Othernode, y_scores_train_tHQ_sample_Othernode, y_scores_train_newphysics_sample_Othernode]
        self.yscores_train_non_categorised[1] = [y_scores_train_Other_sample_ttWnode, y_scores_train_ttW_sample_ttWnode, y_scores_train_tHQ_sample_ttWnode, y_scores_train_newphysics_sample_ttWnode]
        self.yscores_train_non_categorised[2] = [y_scores_train_Other_sample_tHQnode, y_scores_train_ttW_sample_tHQnode, y_scores_train_tHQ_sample_tHQnode, y_scores_train_newphysics_sample_tHQnode]
        self.yscores_train_non_categorised[3] = [y_scores_train_Other_sample_newphysicsnode, y_scores_train_ttW_sample_newphysicsnode, y_scores_train_tHQ_sample_newphysicsnode, y_scores_train_newphysics_sample_newphysicsnode]
         
        self.yscores_test_non_categorised[0] = [y_scores_test_Other_sample_Othernode, y_scores_test_ttW_sample_Othernode, y_scores_test_tHQ_sample_Othernode, y_scores_test_newphysics_sample_Othernode]
        self.yscores_test_non_categorised[1] = [y_scores_test_Other_sample_ttWnode, y_scores_test_ttW_sample_ttWnode, y_scores_test_tHQ_sample_ttWnode, y_scores_test_newphysics_sample_ttWnode]
        self.yscores_test_non_categorised[2] = [y_scores_test_Other_sample_tHQnode, y_scores_test_ttW_sample_tHQnode, y_scores_test_tHQ_sample_tHQnode, y_scores_test_newphysics_sample_tHQnode]
        self.yscores_test_non_categorised[3] = [y_scores_test_Other_sample_newphysicsnode, y_scores_test_ttW_sample_newphysicsnode, y_scores_test_tHQ_sample_newphysicsnode, y_scores_test_newphysics_sample_newphysicsnode]

        node_name = ['Other','ttW','tHQ','newphysics']
        counter = 0
        for y_scorestrain,y_scorestest in zip(self.yscores_train_categorised,self.yscores_test_categorised):
            colours = ['steelblue','g','Fuchsia','darkgoldenrod']
