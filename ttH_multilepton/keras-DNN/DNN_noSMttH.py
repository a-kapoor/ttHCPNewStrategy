...
from plotting.plotter_noSMttH import plotter
...
def load_data(inputPath,variables,criteria):
    ...
    for key in keys :
            ...
            if tree is not None :
                ...
                try: chunk_arr = tree2array(tree=tree, selection=criteria) # Can use  start=first entry, stop = final entry desired
                    ...
                    if sampleName=='newphysics':
                        chunk_df['sampleWeight'] = 2.13540990638 
        ...
        nttJ = len(data.iloc[(data.target.values == 0) & (data.key.values==key) ])
        nttW = len(data.iloc[(data.target.values == 1) & (data.key.values==key) ])
        nTHQ = len(data.iloc[(data.target.values == 2) & (data.key.values==key) ])
        nnewphysics = len(data.iloc[(data.target.values == 3) & (data.key.values==key) ])
        nOther = len(data.iloc[(data.target.values == 4) & (data.key.values==key) ])
    ...
    n = len(data)
    nttJ = len(data.iloc[data.target.values == 0])
    nttW = len(data.iloc[data.target.values == 1])
    nTHQ = len(data.iloc[data.target.values == 2])
    nnewphysics = len(data.iloc[data.target.values == 3])
    nOther = len(data.iloc[data.target.values == 4])
    print("Total length of ttJ = %i, nttW = %i, nTHQ = %i, nnewphysics = %i" % (nttJ , nttW, nTHQ, nnewphysics))
    return data
...
def main():
    ...
    output_directory = '/publicfs/cms/user/yuancc/DarkMatter/MUSICsim/ttHnew/ttH_multilepton/keras-DNN/result/NoSMttH_%s_%s/' % (selection,classweights_name)
...
# Create confusion matrices for training and testing performance                                                                                                                                                                                                            
    ...
    Plotter.ROC_sklearn(original_encoded_train_Y, result_probs, original_encoded_test_Y, result_probs_test, 0 , 'Other_model')
    Plotter.ROC_sklearn(original_encoded_train_Y, result_probs, original_encoded_test_Y, result_probs_test, 1 , 'ttWnode_model')
    Plotter.ROC_sklearn(original_encoded_train_Y, result_probs, original_encoded_test_Y, result_probs_test, 2 , 'tHQnode_model')
    Plotter.ROC_sklearn(original_encoded_train_Y, result_probs, original_encoded_test_Y, result_probs_test, 3 , 'newphysicsnode_model')
